...
const OrderAppointments = () => {
  ...
  const fetchReservedDates = useCallback(async (selectedDate) => {
    ...
    try {
      ...
      const reservedSlotsWithDate =
        response.data.reservedSlots.map((slot) => ({
          ...slot,
          date: formattedDate,
        })) || [];
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (error.response && error.response.status === 409) {
        ...
        alert(
          "Virhe tallennettaessa tapaamista: " +
            error.message +
            " Tapaamisen tallentaminen epäonnistui. Yritä uudelleen.",
        );
      }
    }
  };
  ...
  return (
    <div className="orderAppointments">
      ...
      {/* Ajanvarauslomake */}
      <div className="make_appointment_div">
        {!showTimeInput ? (
